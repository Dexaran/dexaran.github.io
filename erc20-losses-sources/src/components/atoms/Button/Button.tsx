...
export const WhiteButton = ({ children, isLoading, onClick, ...rest }: ButtonProps) => {
  return (
    <button
      className={`${styles.button} ${isLoading ? styles.isLoading : ""} ${styles.white}`}
      onClick={!isLoading ? onClick : undefined}
      {...rest}
    >
      <span>{children}</span>
...
export const WhiteSecondaryButton = ({ children, isLoading, onClick, ...rest }: ButtonProps) => {
  return (
    <button
      className={`${styles.button} ${isLoading ? styles.isLoading : ""} ${styles.whiteSecondary}`}
      onClick={!isLoading ? onClick : undefined}
      {...rest}
    >
      <span>{children}</span>
